...
def main():
    ...
    iface = str(sys.argv[4]) 
    src_addr = socket.gethostbyname(sys.argv[5])
    dst_addr = socket.gethostbyname(sys.argv[6])
    host_mac_addr =  ni.ifaddresses(iface)[ni.AF_LINK][0]['addr']

    print(("sending on interface %s to %s" % (iface, str(dst_addr))))
    pkt =  Ether(src=host_mac_addr, dst='00:00:00:00:00:02')
    pkt = pkt /IP(src=src_addr,dst=dst_addr) / TCP(dport=1234, sport=random.randint(49152,65535)) / sys.argv[5]
    pkt.show2()
    sendp(pkt, iface=iface, verbose=False)
